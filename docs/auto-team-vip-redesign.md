...
`POST /v2/health`：请求头 `X-Admin-Token` 与 Worker secret 比对，请求体 `{ "period_id": "…" }`。返回该期 DO 的聚合指标——开放/满员队伍数、活跃租约数、失败冷却中队伍数、401/限速计数。仅聚合、不含 uid/code 明细；校验失败返回 404（不暴露端点存在性）。供运维监控与容量观察，非客户端功能依赖。
