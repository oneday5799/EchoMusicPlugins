...
2. Upsert `created` 队（captain_uid = uid）与 `joined` 队（captain 取上报值，外部码 captain 可为空串）→ 更新 `member_count`、`snapshot_at`、status。
...
1. **酷狗是否存在"退出队伍"接口？** 若存在，v2.x 可实现"死队换绑"：加入永远无法满员的队伍后自动退出重匹配（提升体验但增加复杂度，v2 暂不做）。
2. **队员奖励 5 天/3 天是否与加入顺序相关？** 日志推断相关（先加入得 5 天）。本设计默认无关、仍采用快满优先；若确认相关且你在意个体激励，可加"空队优先"开关。
3. **`my_join_team_list` 是否可能多于 1 支队伍？** 当前协议按最多 1 支设计（与 v1 行为一致）；若官方允许多支，快照结构可平滑扩展为数组。
