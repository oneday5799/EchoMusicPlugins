...
- **📋 平铺展示**：开启后递归扫描所有子目录，将全部音乐集中展示在一个列表中，扫描进度在任务中心实时显示
...
| 密码 | Basic Auth 密码 | - |
| 根目录路径 | 浏览音乐的起始路径 | `/` |
| 平铺展示 | 开启后递归扫描所有子目录，集中展示全部音乐，扫描进度在任务中心显示 | 关闭 |
...
- **新增平铺展示模式**：每个库可独立开启，递归扫描所有子目录将音乐集中展示在一个列表中，支持搜索、排序、批量操作、右键菜单和完整播放流程
- **任务中心集成**：平铺扫描进度在任务中心实时显示，支持取消操作
- **平铺缓存**：扫描结果持久缓存，返回时无需重新扫描，手动刷新按钮可强制重新扫描
- 修复缓存比较路径不一致导致每次返回都重新扫描的问题
- 修复平铺模式下单曲播放 URL 构造错误的问题
- 修复平铺模式下歌手显示为未知的问题
- 代码优化：移除死代码、提取共享排序比较器、使用正确的 storage API

### v1.3.7

- 移除插件启用时的 "WebDAV 音乐已启用" 提示窗
